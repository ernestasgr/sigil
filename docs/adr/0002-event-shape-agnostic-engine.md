...
- **EventPayloadSchemaRegistry now** — a map from event name to Zod payload schema, introduced in this slice with one member (`file.*` → `FileEventPayloadSchema`). Rejected because the engine doesn't need it (it uses `typeof`, not schema introspection). The registry's real consumers are the UI (showing available fields when building a workflow) and manifest validation (ensuring plugins emit valid events) — neither exists yet. Deferring the registry until it has a real consumer avoids speculative design with one member.
