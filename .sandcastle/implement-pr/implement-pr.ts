...
const prViewJson = sh(`gh pr view ${PR_NUMBER} --json title,body,headRefOid,comments`);
...
const reviewsJson = sh(`gh api repos/{owner}/{repo}/pulls/${PR_NUMBER}/reviews`);
const reviews = z
    .array(
        z.object({
            id: z.number(),
...
if (commitsThisRun === 0 && replyCount === 0) {
    fail('Agent produced no commits and no replies — nothing to do for the unresolved feedback.');
}
...
fs.writeFileSync(path.join(OUTPUT_DIR, 'has_commits.txt'), commitsThisRun > 0 ? 'true' : 'false');
